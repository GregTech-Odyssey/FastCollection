...
/**
 * A concurrent cache from primitive {@code long} keys to values held by
 ...
 *
 * <p>{@link #getCache(int)} / {@link #getCache(int, LongFunction)} run the create
 ...
 */
public final class WeakValueLongCache<V> extends OpenCacheTable<WeakValueLongCache.Node<V>> implements ICleanableCache {
    ...
    public V getCache(final long k) {
        return getCache(k, HashCommon.mix(Long.hashCode(k)), createFunction);
    }
    ...
    public V getIfPresent(final long k) {
        return getIfAbsent(k, HashCommon.mix(Long.hashCode(k)));
    }
    ...
    public V putIfAbsent(final long k, final V v) {
        return putIfAbsent(k, v, HashCommon.mix(Long.hashCode(k)));
    }
    ...
    protected int hashOf(Node<V> node) {
        return HashCommon.mix(Long.hashCode(node.key));
    }
     ...
     */
    @SuppressWarnings("unchecked")
    private V putIfAbsent(final long k, final V v, final int mix) {
