...
/**
 * A concurrent cache from primitive {@code float} keys to strong object values.
 ...
 *
 * <p>{@link #getCache(int)} / {@link #getCache(int, Float2ObjectFunction)} run the
 ...
 */
public final class FloatCache<V> extends OpenCacheTable<FloatCache.Node<V>> {
    ...
    private V getIfAbsent(final float k, final int mix) {
        ...
        for (int steps = 0; steps <= mask; steps++, index = (index + 1) & mask) {
            ...
            if (Float.floatToIntBits(node.key) == Float.floatToIntBits(k)) return node.value;
        }
