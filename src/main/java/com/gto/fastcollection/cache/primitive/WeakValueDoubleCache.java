...
/**
 * A concurrent cache from primitive {@code double} keys to values held by
 ...
 *
 * <p>{@link #getCache(int)} / {@link #getCache(int, DoubleFunction)} run the create
 ...
 */
public final class WeakValueDoubleCache<V> extends OpenCacheTable<WeakValueDoubleCache.Node<V>> implements ICleanableCache {
    ...
    private V getIfAbsent(final double k, final int mix) {
        ...
        for (int steps = 0; steps <= mask; steps++, index = (index + 1) & mask) {
            ...
            if (Double.doubleToLongBits(node.key) == Double.doubleToLongBits(k)) return node.get();
        }
    ...
    private V getCache(final double k, final int mix, DoubleFunction<? extends V> createFunction) {
        ...
        for (int steps = 0; steps <= mask; steps++, index = (index + 1) & mask) {
            ...
            if (Double.doubleToLongBits(node.key) == Double.doubleToLongBits(k)) {
