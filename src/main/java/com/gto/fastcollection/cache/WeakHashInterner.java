 ...
 * instances alive on its own 闂?suitable when callers create many short-lived
 ...
 * will create a fresh one). Dead nodes are reused by writers, reported through
 * the reference queue and dropped by {@link CacheCleaner} through
 * {@link #clearCache()}.
 *
 * <p>Reads are lock-free; insertions compare-and-set on the slot value the probe
 * observed, and a lock is taken only to resize.
 */
public final class WeakHashInterner<T> extends OpenCacheTable<WeakReferenceNode<T>> implements Interner<T>, ICleanableCache {

    /**
     * Creates an interner; registers it with {@link CacheCleaner}.
     */
     ...
     * (weak referent, stored hash) whose referent is both the key and the value.
     */
    @Override
    ...
    private T lookup(final T k, final int hash, final int mix) {
        ...
        for (int steps = 0; steps <= mask; steps++, index = (index + 1) & mask) {
            ...
            if (key != null && node.hash == hash && (key == k || k.equals(key))) return key;
        }
        ...
        for (; ; ) {
            ...
            try {
                ...
                for (int steps = 0; steps <= mask; steps++, index = (index + 1) & mask) {
                    ...
                    if (node.hash == hash && (key == k || k.equals(key))) return key;
                }
                if (vacant < 0) {
