...
/**
 * A cached-hash open-addressing map from object keys to object values. Each
 * slot stores the key's hash in a parallel {@code int[]} array, so probes
 * short-circuit on the stored hash and reference identity ({@code ==}) before
 * falling back to {@code equals}; {@code rehash} reuses the stored hashes
