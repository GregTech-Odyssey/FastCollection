package com.gto.fastcollection.map.enums;

import com.gto.fastcollection.util.EnumKeys;
import it.unimi.dsi.fastutil.objects.*;
import it.unimi.dsi.fastutil.ints.IntCollection;
import it.unimi.dsi.fastutil.ints.IntIterator;
import it.unimi.dsi.fastutil.ints.AbstractIntCollection;

import java.util.Arrays;
import java.util.Map;
import java.util.NoSuchElementException;
import java.util.function.BiConsumer;
import java.util.function.Consumer;

/**
 * A {@link Reference2IntMap} with enum keys, indexed by key ordinal exactly
 * like {@link java.util.EnumMap}: every operation is a single array access
 * with no hashing, and the {@code getInt}/put/remove paths never autobox.
 *
 * <p>Like {@code EnumMap}, a single value array decides presence: the
 * primitive zero value {@code 0} marks an empty slot (the analogue of
 * {@code EnumMap}'s {@code null}), so <b>storing the zero value removes the
 * mapping</b>, and {@link #containsKey} distinguishes presence from absence.
 * Reads of absent keys return {@link #defRetValue}, which defaults
 * to the zero value and can be changed with
 * {@link #defaultReturnValue(int)}. When zero is a meaningful value, use
 * {@link Enum2ObjectMap} instead.
 *
 * <p>The entry-set iterator reuses a single mutable entry (as fastutil's
 * fast iterators do): entries are valid only until the next
 * {@code next()} call.
 *
 * <p>Null keys read as absent; keys of a foreign enum type read as absent.
 * Not thread-safe.
 *
 * @param <K> the enum key type
 */
public final class Enum2IntMap<K extends Enum<K>> extends AbstractReference2IntMap<K> {

    private final Class<K> keyType;
    private final K[] keys;
    private final int[] vals;
    private int size;

    private final EntrySet entrySet = new EntrySet();
    private final ReferenceSet<K> keySet = new KeySet();
    private final IntCollection values = new Values();

    /**
     * Creates an empty map able to hold every constant of {@code keyType}.
     */
    public Enum2IntMap(Class<K> keyType) {
        this.keyType = keyType;
        this.keys = EnumKeys.universe(keyType);
        this.vals = new int[keys.length];
    }

    /**
     * Creates a copy of {@code map}.
     */
    public Enum2IntMap(Enum2IntMap<K> map) {
        this.keyType = map.keyType;
        this.keys = map.keys;
        this.vals = map.vals.clone();
        this.size = map.size;
    }

    /**
     * The enum type this map is indexed by.
     */
    public Class<K> keyType() {
        return keyType;
    }

    @Override
    public int getInt(Object key) {
        if (!isValidKey(key)) return defRetValue;
        int v = vals[((Enum<?>) key).ordinal()];
        return v != 0 ? v : defRetValue;
    }

    @Override
    public int getOrDefault(Object key, int defaultValue) {
        if (!isValidKey(key)) return defaultValue;
        int v = vals[((Enum<?>) key).ordinal()];
        return v != 0 ? v : defaultValue;
    }

    /**
     * Binds {@code value} to {@code key}. Storing the zero value removes the
     * mapping (single-array presence, like {@code EnumMap}).
     *
     * @return the previous value, or {@link #defRetValue} if absent
     */
    @Override
    public int put(K key, int value) {
        int i = key.ordinal();
        int old = vals[i];
        vals[i] = value;
        if (value == 0) {
            if (old != 0) size--;
        } else if (old == 0) {
            size++;
        }
        return old != 0 ? old : defRetValue;
    }

    @Override
    public int removeInt(Object key) {
        if (!isValidKey(key)) return defRetValue;
        int i = ((Enum<?>) key).ordinal();
        int old = vals[i];
        if (old == 0) return defRetValue;
        vals[i] = 0;
        size--;
        return old;
    }

    @Override
    public boolean containsKey(Object key) {
        return isValidKey(key) && vals[((Enum<?>) key).ordinal()] != 0;
    }

    @Override
    public boolean containsValue(int value) {
        if (value == 0) return false;
        final var vals = Enum2IntMap.this.vals;
        final int length = vals.length;
        for (int i = 0; i < length; i++) {
            if (vals[i] == value) return true;
        }
        return false;
    }

    @Override
    public int size() {
        return size;
    }

    @Override
    public void clear() {
        if (size == 0) return;
        Arrays.fill(vals, 0);
        size = 0;
    }

    @Override
    public void forEach(BiConsumer<? super K, ? super Integer> action) {
        final var vals = Enum2IntMap.this.vals;
        final int length = vals.length;
        for (int i = 0; i < length; i++) {
            if (vals[i] != 0) action.accept(keys[i], Integer.valueOf(vals[i]));
        }
    }

    @Override
    public FastEntrySet<K> reference2IntEntrySet() {
        return entrySet;
    }

    @Override
    public ReferenceSet<K> keySet() {
        return keySet;
    }

    @Override
    public IntCollection values() {
        return values;
    }

    /**
     * Binds {@code value} to {@code key} only if the key is absent.
     *
     * @return the existing value, or {@code defRetValue} if the key was absent
     */
    @Override
    public int putIfAbsent(K key, int value) {
        int i = key.ordinal();
        int old = vals[i];
        if (old != 0) return old;
        if (value != 0) {
            vals[i] = value;
            size++;
        }
        return defRetValue;
    }

    @Override
    public boolean remove(Object key, int value) {
        if (value == 0 || !isValidKey(key)) return false;
        int i = ((Enum<?>) key).ordinal();
        if (vals[i] != value) return false;
        vals[i] = 0;
        size--;
        return true;
    }

    @Override
    public boolean replace(K key, int oldValue, int newValue) {
        if (oldValue == 0 || vals[key.ordinal()] != oldValue) return false;
        put(key, newValue);
        return true;
    }

    @Override
    public int replace(K key, int value) {
        int old = vals[key.ordinal()];
        if (old == 0) return defRetValue;
        put(key, value);
        return old;
    }

    @Override
    public int computeIfAbsent(K key, java.util.function.ToIntFunction<? super K> mappingFunction) {
        int i = key.ordinal();
        int v = vals[i];
        if (v != 0) return v;
        int newValue = mappingFunction.applyAsInt(key);
        if (newValue != 0) {
            vals[i] = newValue;
            size++;
        }
        return newValue;
    }

    @Override
    public int computeIfAbsent(K key, Reference2IntFunction<? super K> mappingFunction) {
        int i = key.ordinal();
        int v = vals[i];
        if (v != 0) return v;
        int newValue = mappingFunction.getInt(key);
        if (newValue != 0) {
            vals[i] = newValue;
            size++;
        }
        return newValue;
    }

    @Override
    public int computeIntIfPresent(K key, java.util.function.BiFunction<? super K, ? super Integer, ? extends Integer> remappingFunction) {
        int i = key.ordinal();
        int old = vals[i];
        if (old == 0) return defRetValue;
        Integer newValue = remappingFunction.apply(key, Integer.valueOf(old));
        if (newValue == null) {
            vals[i] = 0;
            size--;
            return defRetValue;
        }
        put(key, newValue);
        return newValue;
    }

    @Override
    public int computeInt(K key, java.util.function.BiFunction<? super K, ? super Integer, ? extends Integer> remappingFunction) {
        int i = key.ordinal();
        int old = vals[i];
        Integer newValue = remappingFunction.apply(key, old == 0 ? null : Integer.valueOf(old));
        if (newValue == null) {
            if (old != 0) {
                vals[i] = 0;
                size--;
            }
            return defRetValue;
        }
        put(key, newValue);
        return newValue;
    }

    /**
     * Adds {@code incr} to the value of {@code key} and stores the sum; an
     * absent key starts from {@code 0}. A sum of {@code 0} removes the
     * mapping (the zero-value sentinel).
     *
     * @return the previous value, or {@code defRetValue} if the key was absent
     */
    public int addTo(K key, int incr) {
        int i = key.ordinal();
        int old = vals[i];
        if (old == 0) {
            if (incr == 0) return defRetValue;
            vals[i] = incr;
            size++;
            return defRetValue;
        }
        int sum = old + incr;
        vals[i] = sum;
        if (sum == 0) size--;
        return old;
    }

    @Override
    public int mergeInt(K key, int value, java.util.function.IntBinaryOperator remappingFunction) {
        int i = key.ordinal();
        int old = vals[i];
        int newValue = old == 0 ? value : remappingFunction.applyAsInt(old, value);
        if (newValue == 0) {
            if (old != 0) {
                vals[i] = 0;
                size--;
            }
            return defRetValue;
        }
        if (old == 0) size++;
        vals[i] = newValue;
        return newValue;
    }

    private boolean isValidKey(Object key) {
        if (key == null) return false;
        Class<?> keyClass = key.getClass();
        return keyClass == keyType || keyClass.getSuperclass() == keyType;
    }

    private final class EntrySet extends AbstractObjectSet<Reference2IntMap.Entry<K>>
            implements Reference2IntMap.FastEntrySet<K> {

        @Override
        public ObjectIterator<Reference2IntMap.Entry<K>> iterator() {
            return new EntryIterator();
        }

        @Override
        public ObjectIterator<Reference2IntMap.Entry<K>> fastIterator() {
            return new EntryIterator();
        }

        /**
         * Feeds a single reused entry to {@code consumer}: no per-entry allocation.
         */
        @Override
        public void fastForEach(Consumer<? super Reference2IntMap.Entry<K>> consumer) {
            Entry entry = new Entry();
            final var vals = Enum2IntMap.this.vals;
            final int length = vals.length;
            for (int i = 0; i < length; i++) {
                if (vals[i] != 0) {
                    entry.index = i;
                    consumer.accept(entry);
                }
            }
        }

        @Override
        public int size() {
            return size;
        }

        @Override
        public void clear() {
            Enum2IntMap.this.clear();
        }

        @Override
        public boolean contains(Object o) {
            if (!(o instanceof Map.Entry<?, ?> e)) return false;
            if (!isValidKey(e.getKey())) return false;
            int i = ((Enum<?>) e.getKey()).ordinal();
            return vals[i] != 0 && Integer.valueOf(vals[i]).equals(e.getValue());
        }

        @Override
        public boolean remove(Object o) {
            if (!contains(o)) return false;
            removeInt(((Map.Entry<?, ?>) o).getKey());
            return true;
        }
    }

    /**
     * A reused, index-backed entry; valid only while its iterator stands still.
     */
    private final class Entry implements Reference2IntMap.Entry<K> {

        private int index = -1;

        @Override
        public K getKey() {
            return keys[index];
        }

        @Override
        public int getIntValue() {
            return vals[index];
        }

        @Override
        public int setValue(int value) {
            int old = vals[index];
            put(keys[index], value);
            return old;
        }

        @Override
        public boolean equals(Object o) {
            if (!(o instanceof Map.Entry<?, ?> e)) return false;
            return e.getKey() == keys[index] && Integer.valueOf(vals[index]).equals(e.getValue());
        }

        @Override
        public int hashCode() {
            return keys[index].hashCode() ^ Integer.valueOf(vals[index]).hashCode();
        }

        @Override
        public String toString() {
            return keys[index] + "=" + Integer.valueOf(vals[index]);
        }
    }

    private abstract class IndexIterator {

        int cursor;
        int current = -1;

        public boolean hasNext() {
            final var vals = Enum2IntMap.this.vals;
            final int length = vals.length;
            while (cursor < length && vals[cursor] == 0) {
                cursor++;
            }
            return cursor < length;
        }

        int advance() {
            if (!hasNext()) throw new NoSuchElementException();
            return current = cursor++;
        }

        public void remove() {
            if (current < 0) throw new IllegalStateException();
            Enum2IntMap.this.put(keys[current], 0);
            current = -1;
        }
    }

    private final class EntryIterator extends IndexIterator implements ObjectIterator<Reference2IntMap.Entry<K>> {

        private final Entry entry = new Entry();

        @Override
        public Reference2IntMap.Entry<K> next() {
            entry.index = advance();
            return entry;
        }
    }

    private final class KeyIterator extends IndexIterator implements ObjectIterator<K> {

        @Override
        public K next() {
            return keys[advance()];
        }
    }

    private final class ValueIterator extends IndexIterator implements IntIterator {

        @Override
        public int nextInt() {
            return vals[advance()];
        }
    }

    private final class KeySet extends AbstractReferenceSet<K> {

        @Override
        public ObjectIterator<K> iterator() {
            return new KeyIterator();
        }

        @Override
        public int size() {
            return size;
        }

        @Override
        public void clear() {
            Enum2IntMap.this.clear();
        }

        @Override
        public boolean contains(Object o) {
            return containsKey(o);
        }

        @Override
        public boolean remove(Object o) {
            if (!containsKey(o)) return false;
            Enum2IntMap.this.removeInt(o);
            return true;
        }
    }

    private final class Values extends AbstractIntCollection {

        @Override
        public IntIterator iterator() {
            return new ValueIterator();
        }

        @Override
        public boolean contains(int value) {
            return Enum2IntMap.this.containsValue(value);
        }

        @Override
        public int size() {
            return size;
        }

        @Override
        public void clear() {
            Enum2IntMap.this.clear();
        }
    }
}
