package com.gto.fastcollection.cache.legacy.primitive;

import com.gto.fastcollection.Concurrents;
import com.gto.fastcollection.cache.CacheCleaner;
import com.gto.fastcollection.cache.legacy.ChainNode;
import com.gto.fastcollection.cache.legacy.HashSegment;
import com.gto.fastcollection.cache.ICleanableCache;
import com.gto.fastcollection.cache.legacy.Segmented;
...
/**
 * A segmented concurrent cache from primitive {@code byte} keys to weak
 ...
 * @see ByteCache
 */
public final class WeakValueByteCache<V> extends Segmented<WeakValueByteCache.Segment<V>> implements ICleanableCache {

    private final Byte2ObjectFunction<? extends V> createFunction;

    /**
     * Creates a cache with default concurrency and no default create function.
     */
    public WeakValueByteCache() {
     ...
     * constructor.
     */
    public WeakValueByteCache(Byte2ObjectFunction<? extends V> createFunction) {
     ...
     */
    public WeakValueByteCache(int concurrencyLevel) {
     ...
     * @throws IllegalArgumentException if {@code concurrencyLevel} is not positive
     */
    public WeakValueByteCache(int concurrencyLevel, Byte2ObjectFunction<? extends V> createFunction) {
     ...
     */
    public V getCache(final byte k) {
     ...
     */
    public V getCache(final byte k, Byte2ObjectFunction<? extends V> createFunction) {
        int mix = HashCommon.mix(k);
        return segmentFor(mix).getCache(k, mix, createFunction);
    }
     ...
     */
    public V getIfPresent(final byte k) {
        int mix = HashCommon.mix(k);
    ...
    static final class Segment<V> extends HashSegment<Node<V>> {
         ...
         */
        private V getCache(final byte k, final int mix, Byte2ObjectFunction<? extends V> createFunction) {
            long stamp = readLock();
            try {
         ...
         */
        private V getIfAbsent(final byte k, final int mix) {
            long stamp = readLock();
            try {
                Node<V> curr = table[mix & mask];
                while (curr != null) {
         ...
         */
        private V put(final byte k, final V v, final int mix) {
    ...
    static final class Node<V> extends WeakReference<V> implements ChainNode<Node<V>> {
        ...
        private Node(byte key, V value, Node<V> next) {
            super(value);
            this.key = key;
            this.next = next;
        }
